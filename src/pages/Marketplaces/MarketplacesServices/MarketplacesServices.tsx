...
const MarketplacesServices = () => {
    ...
    if (!inputRef.current?.value.trim()) {
      ...
      try {
        const res = await axios.get(
          `${BaseUrl}?search=${inputRef.current?.value.trim()}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  const SortByIncrease = async (
    ...
    try {
      const res = await axios.get(`${BaseUrl}/users?sortBy=price&sort=asc`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  const SortByDecrease = async (
    ...
    try {
      const res = await axios.get(`${BaseUrl}/users?sortBy=price&sort=desc`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
